...
if __name__ == "__main__":
    bench_parameters = dict(
        batch_size='auto',  # batch_size='auto' by default
        # memmap_input=True,
        # backend='threading',  # backend='multiprocessing' by default
        input_data_size=int(1e7),  # input output data size in bytes
        output_data_size=int(1e3),  # input output data size in bytes
        n_jobs=2,
    )
    print("Common benchmark parameters:")
    pprint(bench_parameters)

    parallel.MIN_IDEAL_BATCH_DURATION = 0.1
    parallel.MAX_IDEAL_BATCH_DURATION = 0.5
    ...
    high_variance = np.random.normal(loc=0.00001, scale=0.001, size=10000)
